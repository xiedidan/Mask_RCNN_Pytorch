...
def unique1d(tensor):
    ...
    if tensor.is_cuda:
        first_element = first_element.cuda(device=tensor.device)
...
def proposal_generator(inputs, proposal_count, nms_threshold, anchors, config=None):
    ...
    if config.GPU_COUNT:
        std_dev = std_dev.cuda(device=deltas.device)
    ...
    if config.GPU_COUNT:
        norm = norm.cuda(device=boxes.device)
...
def pyramid_roi_align(inputs, pool_size, image_shape):
    ...
    if boxes.is_cuda:
        image_area = image_area.cuda(device=boxes.device)
    ...
    for i, level in enumerate(range(2, 6)):
        ...
        if level_boxes.is_cuda:
            ind = ind.cuda(device=level_boxes.device)
...
def bbox_overlaps(boxes1, boxes2):
    ...
    if y1.is_cuda:
        zeros = zeros.cuda(device=y1.device)
...
def detection_target_layer(proposals, gt_class_ids, gt_boxes, gt_masks, config):
    ...
    if torch.nonzero(gt_class_ids < 0).size()[0]:
        ...
        if config.GPU_COUNT:
            no_crowd_bool = no_crowd_bool.cuda(device=proposals.device)
    ...
    if torch.nonzero(positive_roi_bool).size()[0]:
        ...
        if config.GPU_COUNT:
            rand_idx = rand_idx.cuda(device=proposals.device)
        ...
        if config.GPU_COUNT:
            std_dev = std_dev.cuda(device=proposals.device)
        deltas /= std_dev
        ...
        if config.GPU_COUNT:
            box_ids = box_ids.cuda(device=boxes.device)
    ...
    if torch.nonzero(negative_roi_bool).size()[0] and positive_count>0:
        ...
        if config.GPU_COUNT:
            rand_idx = rand_idx.cuda(device=boxes.device)
    ...
    if positive_count > 0 and negative_count > 0:
        ...
        if config.GPU_COUNT:
            zeros = zeros.cuda(device=boxes.device)
        ...
        if config.GPU_COUNT:
            rois = rois.cuda(device=proposals.device)
            roi_gt_class_ids = roi_gt_class_ids.cuda(device=proposals.device)
            deltas = deltas.cuda(device=proposals.device)
            masks = masks.cuda(device=proposals.device)
...
def refine_detections(rois, probs, deltas, window, config):
    ...
    if config.GPU_COUNT:
        idx = idx.cuda(device=rois.device)
    ...
    std_dev = Variable(torch.from_numpy(np.reshape(config.RPN_BBOX_STD_DEV, [1, 4])).float(), requires_grad=False)
    if config.GPU_COUNT:
        std_dev = std_dev.cuda(device=rois.device)
    ...
    if config.GPU_COUNT:
        scale = scale.cuda(device=rois.device)
    refined_rois *= scale
...
def compute_mrcnn_class_loss(target_class_ids, pred_class_logits):
    ...
    if target_class_ids.size()[0]:
        ...
        if target_class_ids.is_cuda:
            loss = loss.cuda(device=target_class_ids.device)
...
class Dataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, image_index):
        ...
        # Do not skip bg samples in medical projects
...
class MaskRCNN(nn.Module):
    ...
    def build(self, config):
        ...
        if self.config.GPU_COUNT:
            self.anchors = self.anchors.to(device=self.device)
    ...
    def set_log_dir(self, model_path=None):
        ...
        self.state_path = os.path.join(self.log_dir, 'mask_rcnn_{}_states.pth'.format(self.config.NAME.lower()))
    ...
    def load_states(self):
        if (os.path.exists(self.state_path)):
            states = torch.load(self.state_path)
            self.loss_history = states.loss_history
            self.val_map_history = states.val_map_history
    ...
    def train_model(self, train_dataset, val_dataset, learning_rate, epochs,BatchSize,steps, layers, augmentation):
        ...
        for epoch in range(self.epoch+1, epochs+1):
            log("\nEpoch {}/{} Training".format(epoch,epochs))
            epoch_loss = 0
            step = 0
            ...
            for inputs in tqdm(train_generator):
                optimizer.zero_grad()
                loss=0
                ...
                for i in range(batch_len):
                    ...
                    if self.config.GPU_COUNT:
                        images = images.to(device=self.device)
                        rpn_match = rpn_match.to(device=self.device)
                        rpn_bbox = rpn_bbox.to(device=self.device)
                        gt_class_ids = gt_class_ids.to(device=self.device)
                        gt_boxes = gt_boxes.to(device=self.device)
                    ...
                    if self.config.GPU_COUNT:
                        scale = scale.cuda(device=self.device)
                    ...
                    if not rois.size()[0]:
                        ...
                        if self.config.GPU_COUNT:
                            mrcnn_class_logits = mrcnn_class_logits.cuda(device=self.device)
                            mrcnn_class = mrcnn_class.cuda(device=self.device)
                            mrcnn_bbox = mrcnn_bbox.cuda(device=self.device)
                            mrcnn_mask = mrcnn_mask.cuda(device=self.device)
                    else:
                        # Network Heads
            ...
            print("===> Epoch {} Training Complete: Avg. Loss: {:.4f}".format(epoch, epoch_loss / step))

            # save model
            torch.save(self.state_dict(), self.checkpoint_path.format(epoch))
            self.loss_history.append(epoch_loss / step)

            # online val - compute mAP
            print('\n===> Epoch {} Eval'.format(epoch))
            self.eval()

            aps = []
            val_image_ids = val_dataset.image_ids
            ...
            print('===> Epoch {} Eval Complete: mAP: {:.4f}'.format(epoch, np.mean(aps)))
            self.val_map_history.append(np.mean(aps))
            ...
            torch.save(states, self.state_path)
